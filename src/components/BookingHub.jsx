...
export default function BookingHub({ currentDestination = 'Tokyo', travelers = [] }) {
  const [activeCategory, setActiveCategory] = useState('hotels'); // 'hotels', 'flights', 'vouchers', 'frameworks'
  ...
  useEffect(() => {
    setConfirmedBookings(getStoredBookings());
  }, []);
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 lg:px-10 py-8 space-y-8">
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-xs space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-2 bg-slate-100 p-1.5 rounded-2xl w-fit">
          ...
          >
            ...
            <span>Vetted Hotels (Demand AI Sandbox)</span>
          </button>
          ...
          >
            ...
            <span>Demand AI Bookings</span>
            {confirmedBookings.filter(b => b.status === 'CONFIRMED').length > 0 && (
      ...
      {/* ==================================================== */}
      {/* 3. HOTELS LISTING (DEMAND AI SANDBOX POWERED) */}
      {/* ==================================================== */}
      {activeCategory === 'hotels' && (
        <div className="space-y-6">
          {/* Demand AI Autonomous Banner */}
          <div className="bg-gradient-to-r from-indigo-950 via-indigo-900 to-purple-950 rounded-3xl p-6 sm:p-8 text-white shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border border-indigo-800/50">
            <div className="space-y-2 max-w-2xl">
              <div className="flex flex-wrap items-center gap-2">
                <span className="px-3 py-1 rounded-full bg-white/20 text-indigo-100 text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 shadow-xs">
                  <Zap className="w-3.5 h-3.5 text-amber-300 fill-amber-300" />
                  Booking.com Demand API • Sandbox v3.2
                </span>
                <span className="text-xs text-indigo-200 font-bold">
                  Zero Human Typing Required
                </span>
              </div>
              <h3 className="text-xl sm:text-2xl font-black text-white tracking-tight">
                Autonomous Mock Booking with Demand AI 🤖🏨
              </h3>
              <p className="text-xs sm:text-sm text-indigo-100/90 leading-relaxed">
                Skip repetitive checkout screens. In 1 click, Aegis Concierge auto-maps Alice, Bob, and Charlie’s passports, room configurations, and pre-authorized ledger straight to the simulated Booking.com Demand API.
              </p>
            </div>

            <button
              onClick={() => handleTriggerAutoBook(REAL_WORLD_HOTELS[0])}
              className="w-full md:w-auto flex items-center justify-center gap-2.5 px-6 py-4 bg-amber-400 hover:bg-amber-300 text-slate-900 font-black text-xs sm:text-sm rounded-2xl shadow-xl transition-all shrink-0 active:scale-95 cursor-pointer"
            >
              <Zap className="w-4 h-4 fill-slate-900" />
              <span>⚡ Auto-Book Squad Top Pick</span>
            </button>
          </div>
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <a
              ...
              className="flex items-center gap-1.5 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold transition-all"
            >
              <span>Search manual on Booking.com</span>
              <ExternalLink className="w-3.5 h-3.5" />
            </a>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {REAL_WORLD_HOTELS.map((hotel) => {
              const existingBooking = isHotelBooked(hotel.name) || isHotelBooked(hotel.id);
                ...
                >
                  <div>
                    <div className="relative aspect-video overflow-hidden bg-slate-100">
                      ...
                      />
                      {existingBooking ? (
                        <span className="absolute top-3 left-3 bg-emerald-600 text-white font-extrabold text-[10px] px-3 py-1 rounded-full shadow-xs flex items-center gap-1">
                          <CheckCircle2 className="w-3 h-3" />
                          CONFIRMED VIA DEMAND AI
                        </span>
                      ) : hotel.recommended ? (
                  ...
                  <div className="p-6 pt-0 space-y-3">
                    ...
                    {/* Action Buttons: 1-Click Demand AI vs Manual Links */}
                    {existingBooking ? (
                      <div className="space-y-2">
                        <button
                          onClick={() => handleViewVoucher(existingBooking)}
                          className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl text-xs font-black transition-all shadow-xs flex items-center justify-center gap-2 cursor-pointer"
                        >
                          <Receipt className="w-4 h-4" />
                          <span>View Official Voucher ({existingBooking.pnr})</span>
                        </button>
                        <div className="text-center text-[10px] text-emerald-700 font-bold">
                          Free cancellation available in sandbox
                        </div>
                      </div>
                    ) : (
                      <div className="space-y-2">
                        <button
                          onClick={() => handleTriggerAutoBook(hotel)}
                          className="w-full py-3.5 bg-gradient-to-r from-indigo-600 via-indigo-700 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-2xl text-xs font-black transition-all shadow-md shadow-indigo-100 flex items-center justify-center gap-2 cursor-pointer active:scale-98"
                        >
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="flex items-center justify-center gap-1.5 py-2 px-2 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-xl text-[11px] font-bold transition-all text-center border border-slate-200/80"
                          >
      ...
      {/* ==================================================== */}
      {/* 4. DEMAND AI CONFIRMED VOUCHERS TAB */}
      {/* ==================================================== */}
      {activeCategory === 'vouchers' && (
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h3 className="text-xl font-black text-slate-900">
                Demand AI Confirmed Reservations ({confirmedBookings.filter(b => b.status === 'CONFIRMED').length})
              </h3>
              <p className="text-xs text-slate-500 mt-0.5">
                Simulated Booking.com Demand API Sandbox v3.2 active bookings and vouchers.
              </p>
            </div>
          ...
          {confirmedBookings.filter(b => b.status === 'CONFIRMED').length === 0 ? (
            <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center space-y-4">
              ...
              <div className="space-y-1">
                <h4 className="font-black text-base text-slate-900">No Reservations Yet</h4>
                <p className="text-xs text-slate-500 max-w-md mx-auto">
                  Click the <strong>⚡ 1-Click Demand AI Auto-Book</strong> button on any hotel to generate an autonomous mock reservation with zero typing.
                </p>
              </div>
              <button
                onClick={() => setActiveCategory('hotels')}
